...
MODEL_FILE_MAP = {
    ...
    "SARIMA (1,0,1)(1,1,1)24": "models/sarima.pkl",
    "Holt-Winters (ETS)": "models/holt_winters.pkl"
}
...
def load_and_preprocess():
    ...
    if os.path.exists(parquet_path):
        ...
        st.error("Missing 'processed_hourly_power.parquet'. Run your training scripts first.")
        st.stop()
...
@st.cache_resource
def get_time_series_forecast(model_name, test_len):
    eval_len = min(test_len, 168)
    fitted_model = load_saved_model(model_name)
    forecast = fitted_model.forecast(steps=eval_len)
...
@st.cache_data
def evaluate_all_leaderboard_models(split_idx_val):
    ...
    p_ets, ets_len = get_time_series_forecast("Holt-Winters (ETS)", len(y_test))
    p_sarima, sarima_len = get_time_series_forecast("SARIMA (1,0,1)(1,1,1)24", len(y_test))
    ...
    return bench_df.sort_values(by="RMSE").reset_index(drop=True)
...
with tab3:
    ...
    with st.spinner("Running inference..."):
        if model_choice in ["Linear Regression", "Random Forest", "XGBoost", "LightGBM"]:
            model = load_saved_model(model_choice)
            preds = model.predict(X_test)
            y_eval = y_test
        elif model_choice in ["SARIMA (1,0,1)(1,1,1)24", "Holt-Winters (ETS)"]:
            preds, eval_len = get_time_series_forecast(model_choice, len(y_test))
...
with tab4:
    ...
    with st.spinner("Benchmarking time series models..."):
        bench_df = evaluate_all_leaderboard_models(split_idx)
        st.dataframe(
